...
ELSE
 ...
 ELSE IF ( KRR == 1 ) THEN                           ! only vapor
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PEMOIST(JIJ, JK) = ZDELTA*PTHLM(JIJ, JK)
    END DO
  END DO
  ...
  IF ( KRRI>0) THEN  ! rc and ri case
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ...
        !
        PEMOIST(JIJ, JK) = ZDELTA * (PTHLM(JIJ, JK) + PLOCPEXNM(JIJ, JK)*(           &
