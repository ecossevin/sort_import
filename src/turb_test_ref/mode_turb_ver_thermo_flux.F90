...
!          ------------
!
USE MODE_SHUMAN_PHY, ONLY : MZM_PHY, DZM_PHY, MXF_PHY, & 
& MYF_PHY, MZF_PHY, DZF_PHY 
USE MODE_PRANDTL, ONLY : D_PHI3DTDZ_O_DDTDZ, M3_WTH_W2TH, D_M3_WTH_W2TH_O_DDTDZ, & 
& M3_WTH_WTH2, D_M3_WTH_WTH2_O_DDTDZ, M3_WTH_W2R, & 
& D_M3_WTH_W2R_O_DDTDZ, M3_WTH_WR2, D_M3_WTH_WR2_O_DDTDZ, & 
& M3_WTH_WTHR, D_M3_WTH_WTHR_O_DDTDZ, D_PSI3DRDZ_O_DDRDZ, & 
& M3_WR_W2R, D_M3_WR_W2R_O_DDRDZ, M3_WR_WR2, & 
& D_M3_WR_WR2_O_DDRDZ, M3_WR_W2TH, D_M3_WR_W2TH_O_DDRDZ, & 
& M3_WR_WTH2, D_M3_WR_WTH2_O_DDRDZ, M3_WR_WTHR, & 
& D_M3_WR_WTHR_O_DDRDZ 
...
ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -TURBN%XCSHF*PPHI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = -TURBN%XCSHF*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
...
!#  !
!#  CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_SUBGRID_WThl )  
!#
!#DO JK=1, IKT
!#  DO JIJ=IIJB, IIJE
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_W_SBG_WThl )  
!#CALL GZ_W_M_PHY(D, PWM,PDZZ, ZGZ_W_M2D_WORK1)
!#CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZGZ_W_M2D_WORK1(:, :)*ZMZF2D_WORK1(:, :), &
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_ddxa_Thl_SBG_UaThl )  
!#CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, -TURBN%XCTP*PSQRT_TKE(:, :)/PLM(:, :)*ZMZF2D_WORK1(:, :), &
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_SUBGRID_WThv, .TRUE. )  
!#IF (KRR>=1) THEN
!#    
!#DO JK=1, IKT
!#  DO JIJ=IIJB, IIJE
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_ddxa_Rt_SBG_UaThl )  
!#END IF
!#  !
...
!#  END DO
!#  DO JIJ=IIJB, IIJE
!#    ZA(JIJ, IKB) = TURBN%XCSHF*PPHI3(JIJ, IKB)*ZKEFF(JIJ, IKB)
!#  END DO
!#
...
IF (KRR /= 0) THEN
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -TURBN%XCSHF*PPSI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDRDZ(JIJ, JK) = -TURBN%XCSHF*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
...
!#    CALL SECOND_MNH(ZTIME1)
!#    CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_SUBGRID_WRt )    
!#
!#DO JK=1, IKT
!#  DO JIJ=IIJB, IIJE
!#    ZSHUGRADWK1_2D(JIJ, JK) = PWM(JIJ, JK)*ZFLXZ(JIJ, JK)
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_W_SBG_WRt )    
!#CALL GZ_W_M_PHY(D, PWM,PDZZ, ZGZ_W_M2D_WORK1)
!#CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZGZ_W_M2D_WORK1(:, :)*ZMZF2D_WORK1(:, :),&
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_ddxa_Thl_SBG_UaRt )    
!#
!#DO JK=1, IKT
!#  DO JIJ=IIJB, IIJE
!#    ZSHUGRADWK1_2D(JIJ, JK) = PDR_DZ(JIJ, JK)*ZFLXZ(JIJ, JK)
...
!#!
!#CALL MZF_PHY(D, ZSHUGRADWK1_2D, ZMZF2D_WORK1)
!#CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZMZF2D_WORK1(:, :), TLES%X_LES_RES_ddxa_Rt_SBG_UaRt )    
!#CALL MZM_PHY(D, PEMOIST, ZMZM2D_WORK1)
!#
!#DO JK=1, IKT
!#  DO JIJ=IIJB, IIJE
...
! yet modify to use length scale and tke at half levels from vdfexcuhl
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK)   = ZWORK2(JIJ, JK)/ PDZZ(JIJ, JK) * &
                                      (-PPHI3(JIJ, JK)*ZWORK3(JIJ, JK)) * TURBN%XCSHF 
    END DO
  END DO
